...
const Index: React.FC<IndexProps> = ({ isInfoPageOpen }) => {
  ...
  const [currentConfessionForMeta, setCurrentConfessionForMeta] = useState<Confession | null>(null);
        ...
        setConfessions((currentConfessions) => {
          const updatedConfessions = currentConfessions.map((confession) => {
            if (confession.id === newComment.confession_id) {
              if (confession.comments.some(c => c.id === newComment.id)) return confession;
              return { ...confession, comment_count: confession.comment_count + 1, comments: [newComment, ...confession.comments] };
            }
            return confession;
          });
  ...
  const fetchConfessionsPage = useCallback(async (
    { category, query, oldestCreatedAtForInfiniteScroll, replace = false, fetchId }:
    { category: string; query: string; oldestCreatedAtForInfiniteScroll?: string; replace?: boolean; fetchId: number }
  ) => {
    setLoadingMore(true);
    try {
      let supabaseQuery = supabase.from("confessions").select("*").order("created_at", { ascending: false });

      if (query) {
        supabaseQuery = supabaseQuery.or(`title.ilike.%${query}%,content.ilike.%${query}%`);
      } else if (category !== "Всички") {
        supabaseQuery = supabaseQuery.eq("category", category);
      }

      if (oldestCreatedAtForInfiniteScroll) {
        supabaseQuery = supabaseQuery.lt("created_at", oldestCreatedAtForInfiniteScroll);
      }
      supabaseQuery = supabaseQuery.limit(CONFESSIONS_PER_PAGE);
  ...
  const fetchSingleConfession = useCallback(async (id: string, slug: string, categoryFromUrl: string, fetchId: number) => {
    try {
      const { data: confessionData, error: confessionError } = await supabase.from("confessions").select("*").eq("id", id).single();
      if (confessionError || !confessionData) throw new Error("Confession not found.");
      if (fetchId !== currentFetchId.current) return null;
      ...
      const { data: commentsData } = await supabase.from("comments").select("*").eq("confession_id", id).order("created_at", { ascending: false });
      if (fetchId !== currentFetchId.current) return null;

      return { ...confessionData, comment_count: commentsData?.length || 0, comments: commentsData || [] };
    } catch (error: any) {
      toast.error("Error fetching confession details: " + error.message);
      const currentCategoryParam = searchParams.get('category');
      let redirectPath = '/';
      if (currentCategoryParam && currentCategoryParam !== "Всички") redirectPath += `?category=${currentCategoryParam}`;
  ...
  useEffect(() => {
    ...
    if (!isInfoPageOpen) {
      ...
      lastNonInfoPageCategoryRef.current = categoryFromUrl;
    } else {
      if (selectedCategory !== lastNonInfoPageCategoryRef.current) setSelectedCategory(lastNonInfoPageCategoryRef.current);
    }
  }, [searchParams, selectedCategory, searchQuery, isInfoPageOpen]);

  useEffect(() => {
    if (authLoading) return;
    if (expandedConfessionId !== paramId) setExpandedConfessionId(paramId || null);
    ...
    const isSearchChanged = lastLoadedContextRef.current?.searchQuery !== currentQuery;
    ...
    const needsFullRefetch = isCategoryChanged || isSearchChanged || isNavigatingToNewConfession || isReturningToListAndListIsEmpty;
      ...
      const loadData = async () => {
        try {
          if (currentParamId) {
            ...
            const combinedConfessions = [...(afterData || []).reverse().map(formatConfession), targetConf, ...(beforeData || []).map(formatConfession)];
            const uniqueConfessions = Array.from(new Map(combinedConfessions.map(c => [c.id, c])).values()).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
            setConfessions(uniqueConfessions);
            latestConfessionsRef.current = uniqueConfessions;
          } else {
            await fetchConfessionsPage({ category: currentCategory, query: currentQuery, replace: true, fetchId });
            setCurrentConfessionForMeta(null);
          }
        } finally {
          if (fetchId === currentFetchId.current) {
            setLoading(false);
            lastLoadedContextRef.current = { category: currentCategory, paramId: currentParamId, searchQuery: currentQuery };
          }
        }
      };
      loadData();
    } else {
      if (lastLoadedContextRef.current?.category !== currentCategory || lastLoadedContextRef.current?.paramId !== currentParamId || lastLoadedContextRef.current?.searchQuery !== currentQuery) {
        lastLoadedContextRef.current = { category: currentCategory, paramId: currentParamId, searchQuery: currentQuery };
      }
    }
  }, [authLoading, selectedCategory, searchQuery, paramId, paramSlug, fetchConfessionsPage, fetchSingleConfession, confessions.length]);
  ...
  useEffect(() => {
    if (page > 0 && hasMoreRef.current && !loadingMoreRef.current) {
      ...
      fetchConfessionsPage({ category: selectedCategory, query: searchQuery, oldestCreatedAtForInfiniteScroll: oldestCreatedAt, fetchId: currentFetchId.current });
    }
  }, [page, fetchConfessionsPage, selectedCategory, searchQuery]);
  ...
  useEffect(() => {
    ...
    if (currentLength > prevLength) {
      if (paramId && page === 0) {
        ...
        if (visibleConfessionCount === prevLength) setVisibleConfessionCount(prev => prev + 1);
      }
  ...
  const handleAnimationComplete = useCallback(() => {
    if (visibleConfessionCount < confessions.length) setVisibleConfessionCount(prev => prev + 1);
  ...
  const handleAddComment = async (confessionId: string, content: string, gender: "male" | "female" | "incognito") => {
    ...
    if (error) toast.error("Error posting comment: " + error.message);
    else {
  ...
  const handleFetchComments = async (confessionId: string) => {
    ...
    if (error) toast.error("Error fetching comments: " + error.message);
    else {
      setConfessions(prev => {
  ...
  const handleConfessionToggle = (confessionId: string, slug: string) => {
    ...
    const currentCategoryParam = searchParams.get('category');
    let newPath = '/';
    if (!isCurrentlyExpanded) newPath = `/confessions/${confessionId}/${slug}`;
    if (currentCategoryParam && currentCategoryParam !== "Всички") newPath += `?category=${currentCategoryParam}`;
  ...
  const handleSelectCategory = (category: string) => {
    let newPath = '/';
    let newSearch = '';
    if (category !== "Всички") newSearch = `?category=${category}`;
  ...
  if (paramId && currentConfessionForMeta) {
    ...
    if (searchParams.get('category') && searchParams.get('category') !== "Всички") metaUrl += `?category=${searchParams.get('category')}`;
  } else if (searchQuery) {
    metaTitle = `${SITE_TITLE} - Търсене за "${searchQuery}"`;
    ...
    <div className="container mx-auto p-4 max-w-3xl">
      ...
      {loading ? (
        <div className="space-y-6 mt-8">
          <ConfessionCardSkeleton /><ConfessionCardSkeleton /><ConfessionCardSkeleton />
        </div>
      ) : (
        <div className="space-y-6">
          {confessions.slice(0, visibleConfessionCount).map((conf) => (
            <ConfessionCard key={conf.id} confession={{ ...conf, timestamp: new Date(conf.created_at), comments: conf.comments.map(c => ({ ...c, timestamp: new Date(c.created_at) })) }} onAddComment={handleAddComment} onLikeConfession={handleLikeConfession} onFetchComments={handleFetchComments} isContentOpen={conf.id === expandedConfessionId} isDirectLinkTarget={conf.id === paramId} onToggleExpand={handleConfessionToggle} onSelectCategory={handleSelectCategory} shouldOpenCommentsOnLoad={conf.id === expandedConfessionId && location.hash === '#comments'} onAnimationComplete={handleAnimationComplete} currentCategory={selectedCategory} />
          ))}
        </div>
      )}
      ...
      {!loading && !loadingMore && !hasMore && confessions.length > 0 && <p className="text-center text-gray-500 dark:text-gray-400 mt-8">Това са всички изповеди.</p>}
      {!loading && confessions.length === 0 && <p className="text-center text-gray-500 dark:text-gray-400 mt-8">{searchQuery ? `Няма намерени резултати за "${searchQuery}".` : "Все още няма изповеди в тази категория."}</p>}
