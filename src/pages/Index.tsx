...
const Index: React.FC<IndexProps> = ({ isInfoPageOpen }) => { // Receive prop
  ...
  const fetchConfessionsPage = useCallback(async (
    ...
    try {
      ...
      if (fetchId !== currentFetchId.current) {
        // console.log(`[Fetch] Ignoring stale fetch results for ID: ${fetchId}. Current is: ${currentFetchId.current}`); // Removed console.log
        return [];
      }
  ...
  const fetchSingleConfession = useCallback(async (id: string, slug: string, categoryFromUrl: string, fetchId: number) => {
    try {
      ...
      if (commentsError) { /* console.error("Error fetching comments for single confession:", commentsError); */ } // Removed console.error
  ...
  useEffect(() => {
    ...
    if (needsFullRefetch) {
      ...
      // console.log(`[Effect] Initiating full re-fetch (ID: ${fetchId}). Reason: ${isCategoryChanged ? 'Category Changed' : isNavigatingToNewConfession ? 'Navigating to New Confession' : 'Returning to List'}`); // Removed console.log
